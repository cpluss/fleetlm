defmodule Fleetlm.Runtime.SessionServerV2 do
  @moduledoc """
  ...
  def init(session_id) do
    ...
    if expected_owner != Node.self() do
      Logger.error("SessionServerV2 #{session_id} started on wrong node. Expected: #{expected_owner}")
      ...
      case load_session(session_id) do
        {:ok, session} ->
          ...
          Logger.info("SessionServerV2 started: #{session_id} on node #{Node.self()} (slot: #{slot}, seq: #{seq})")
  ...
  def terminate(reason, state) do
    Logger.info("SessionServerV2 terminating: #{state.session_id}, reason: #{inspect(reason)}")
