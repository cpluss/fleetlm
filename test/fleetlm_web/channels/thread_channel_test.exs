defmodule FleetlmWeb.ThreadChannelTest do
  ...
  describe "DM channels" do
    ...
    test "joining a DM channel returns message history", %{user_a: user_a, user_b: user_b} do
      ...
      {:ok, reply, _socket} = subscribe_and_join(socket, FleetlmWeb.ThreadChannel, "dm:#{user_a}|#{user_b}")
    ...
    test "sending a DM message broadcasts to channel", %{user_a: user_a, user_b: user_b} do
      {:ok, socket} = connect(FleetlmWeb.UserSocket, %{"participant_id" => user_a})
      {:ok, _reply, socket} = subscribe_and_join(socket, FleetlmWeb.ThreadChannel, "dm:#{user_a}|#{user_b}")
    ...
    test "unauthorized users cannot join DM channels", %{user_a: user_a, user_b: user_b} do
      user_c = "user:charlie"

      {:ok, socket} = connect(FleetlmWeb.UserSocket, %{"participant_id" => user_c})

      assert {:error, %{reason: "unauthorized"}} =
               subscribe_and_join(socket, FleetlmWeb.ThreadChannel, "dm:#{user_a}|#{user_b}")
    end
  end
